...
class InnerKFoldClassifier(object):
    ...
    def run(self):
        logging.debug('Training {0} with (inner) {1}-fold cross-validation'
        ...
        for train_idx, test_idx in skf.split(self.X, self.y):
            ...
            logging.debug('Best param found at split {0}: l2reg = {1} \
...
class KFoldClassifier(object):
    ...
    def run(self):
        # cross-validation
        logging.debug('Training {0} with {1}-fold cross-validation'
                     .format(self.modelname, self.k))
        regs = [10**t for t in range(-5, -1)] if self.usepytorch else \
        ...
        # evaluation
        logging.debug([('reg:' + str(regs[idx]), scores[idx])
                      for idx in range(len(scores))])
        optreg = regs[np.argmax(scores)]
        devaccuracy = np.max(scores)
        logging.debug('Cross-validation : best param found is reg = {0} \
...
class SplitClassifier(object):
    ...
    def run(self):
        logging.debug('Training {0} with standard validation..'
        ...
        optreg = regs[np.argmax(scores)]
        devaccuracy = np.max(scores)
        logging.debug('Validation : best param found is reg = {0} with score \
