...
class SICKRelatednessEval_Supervised(object):
    ...
    def run(self, params, batcher):
        ...
        ke = kendalltau(yhat, self.sick_data['test']['y'])[0]
...
class SICKRelatednessEval(object):
    '''
        this class if to compute STS-B and SICK-R using direct cosine similarity
    '''
    def __init__(self, task_path, seed=1111):
    ...
    def run(self, params, batcher):
        ...
        cos_similarity = lambda s1, s2: np.nan_to_num(cosine(np.nan_to_num(s1), np.nan_to_num(s2)))
        yhat = [cos_similarity(testA[i], testB[i]) for i in range(testA.shape[0])]
...
class SICKEntailmentEval(SICKRelatednessEval):
    def __init__(self, task_path, seed=1111):
        logging.info('Transfer task: SICK-Entailment')
        self.seed = seed
        train = self.loadFile(os.path.join(task_path, 'SICK_train.txt'))
        dev = self.loadFile(os.path.join(task_path, 'SICK_trial.txt'))
        test = self.loadFile(os.path.join(task_path, 'SICK_test_annotated.txt'))
