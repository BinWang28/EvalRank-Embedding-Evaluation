    ...
    def run(self, params, batcher):
        ...
        for dataset in self.datasets:
            ...
            results[dataset] = {'pearson': pearsonr(sys_scores, gs_scores),
                                ...
                                'kendall': kendalltau(sys_scores, gs_scores),
                                'nsamples': len(sys_scores)}
            logging.debug('%s : pearson = %.4f, spearman = %.4f, kendall = %.4f' %
                          (dataset,
                           results[dataset]['pearson'][0],
                           results[dataset]['spearman'][0],
                           results[dataset]['kendall'][0]))
        ...
        list_spr = np.array([results[dset]['spearman'][0] for
                            dset in results.keys()])
        list_ken = np.array([results[dset]['kendall'][0] for
                            dset in results.keys()])
                            
        avg_pearson = np.average(list_prs)
        avg_spearman = np.average(list_spr)
        avg_kendall = np.average(list_ken)
        wavg_pearson = np.average(list_prs, weights=weights)
        wavg_spearman = np.average(list_spr, weights=weights)
        wavg_kendall = np.average(list_ken, weights=weights)

        results['all'] = {'pearson': {'mean': avg_pearson,
                                      'wmean': wavg_pearson},
                          'spearman': {'mean': avg_spearman,
                                       'wmean': wavg_spearman},
                          'kendall': {'mean': avg_kendall,
                                      'wmean': wavg_kendall}}

        logging.debug('ALL (weighted average) : Pearson = %.4f, \
            Spearman = %.4f, Kendall = %.4f' % (wavg_pearson, wavg_spearman, wavg_kendall))
        logging.debug('ALL (average) : Pearson = %.4f, \
            Spearman = %.4f, Kendall = %.4f\n' % (avg_pearson, avg_spearman, avg_kendall))
...
class STS12Eval(STSEval):
    def __init__(self, taskpath, seed=1111):
        logging.info('Transfer task: STS12')
        self.seed = seed
...
class STS13Eval(STSEval):
    ...
    def __init__(self, taskpath, seed=1111):
        logging.info('Transfer task: STS13')
        self.seed = seed
...
class STS14Eval(STSEval):
    def __init__(self, taskpath, seed=1111):
        logging.info('Transfer task: STS14')
        self.seed = seed
...
class STS15Eval(STSEval):
    def __init__(self, taskpath, seed=1111):
        logging.info('Transfer task: STS15')
        self.seed = seed
...
class STS16Eval(STSEval):
    def __init__(self, taskpath, seed=1111):
        logging.info('Transfer task: STS16')
        self.seed = seed
